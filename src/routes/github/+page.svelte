<script lang="ts">
  ...
  import GitHubListItem from '$lib/components/github/GitHubListItem.svelte';
  ...
  $effect(() => {
    githubStore.setSearchQuery(appStore.searchQuery);
  });
  ...
  <div class="github-body" class:detail-open={!!githubStore.activeGithub}>
    <aside class="list-panel">
      ...
      <div class="list-scroll">
        ...
        {:else if githubStore.filteredGithub.length === 0}
          <div class="empty-state" in:fly={{ y: 20, duration: 400 }}>
            ...
            <button class="clear-filters-btn" onclick={() => githubStore.clearFilters()}>
              Clear filters
            </button>
          </div>
        {:else}
          {#each githubStore.filteredGithub as item (item.id)}
            <GitHubListItem
              {item}
              isActive={githubStore.activeGithubId === item.id}
              onclick={() => githubStore.setActiveGithub(item.id)}
              onToggleFavorite={() => githubStore.toggleFavorite(item.id)}
            />
          {/each}
        {/if}
      </div>
      ...
      {:else}
        <div class="no-item-state" in:fly={{ y: 20, duration: 400, delay: 150 }}>
          <div class="empty-icon-wrap-large">
            <Icon icon="simple-icons:github" width={48} height={48} />
          </div>
          <div class="empty-text">
