<script lang="ts">
  ...
  import TerminalListItem from '$lib/components/terminal/TerminalListItem.svelte';
  ...
  $effect(() => {
    terminalStore.setSearchQuery(appStore.searchQuery);
  });
  ...
  <div class="terminal-body" class:detail-open={!!terminalStore.activeTerminal}>
    <aside class="list-panel">
      ...
      <div class="list-scroll">
        {#if appStore.terminal.length === 0}
          <div class="empty-state" in:fly={{ y: 20, duration: 400, delay: 100 }}>
            <div class="empty-icon-wrap">
              <Icon icon="ph:terminal-window" width={32} height={32} />
            </div>
        ...
        {:else if terminalStore.filteredTerminal.length === 0}
          <div class="empty-state" in:fly={{ y: 20, duration: 400 }}>
            ...
            <button class="clear-filters-btn" onclick={() => terminalStore.clearFilters()}>
              Clear filters
            </button>
          </div>
        {:else}
          {#each terminalStore.filteredTerminal as item (item.id)}
            <TerminalListItem
              {item}
              isActive={terminalStore.activeTerminalId === item.id}
              onclick={() => terminalStore.setActiveTerminal(item.id)}
              onToggleFavorite={() => terminalStore.toggleFavorite(item.id)}
            />
          {/each}
        {/if}
      </div>
      ...
      {:else}
        <div class="no-item-state" in:fly={{ y: 20, duration: 400, delay: 150 }}>
          <div class="empty-icon-wrap-large">
            <Icon icon="ph:terminal-window" width={48} height={48} />
          </div>
          <div class="empty-text">
