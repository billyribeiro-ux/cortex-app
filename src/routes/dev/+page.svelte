<script lang="ts">
  ...
  import DevListItem from '$lib/components/dev/DevListItem.svelte';
  ...
  $effect(() => {
    devStore.setSearchQuery(appStore.searchQuery);
  });
  ...
  <div class="dev-body">
    <aside class="list-panel">
      ...
      <div class="list-scroll">
        ...
        {:else if devStore.filteredDev.length === 0}
          <div class="empty-state" in:fly={{ y: 20, duration: 400 }}>
            ...
            <button class="clear-filters-btn" onclick={() => devStore.clearFilters()}>
              Clear filters
            </button>
          </div>
        {:else}
          {#each devStore.filteredDev as item (item.id)}
            <DevListItem
              {item}
              isActive={devStore.activeDevId === item.id}
              onclick={() => devStore.setActiveDev(item.id)}
              onToggleFavorite={() => devStore.toggleFavorite(item.id)}
            />
          {/each}
        {/if}
      </div>
      ...
      {:else}
        <div class="no-item-state" in:fly={{ y: 20, duration: 400, delay: 150 }}>
          <div class="empty-icon-wrap-large">
            <Icon icon="ph:code" width={48} height={48} />
          </div>
          <div class="empty-text">
