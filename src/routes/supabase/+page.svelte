<script lang="ts">
  ...
  import SupabaseListItem from '$lib/components/supabase/SupabaseListItem.svelte';
  ...
  $effect(() => {
    supabaseStore.setSearchQuery(appStore.searchQuery);
  });
  ...
  <div class="supabase-body">
    <aside class="list-panel">
      ...
      <div class="list-scroll">
        {#if appStore.supabase.length === 0}
          <div class="empty-state" in:fly={{ y: 20, duration: 400, delay: 100 }}>
            <div class="empty-icon-wrap">
              <Icon icon="simple-icons:supabase" width={32} height={32} />
            </div>
        ...
        {:else if supabaseStore.filteredSupabase.length === 0}
          <div class="empty-state" in:fly={{ y: 20, duration: 400 }}>
            ...
            <button class="clear-filters-btn" onclick={() => supabaseStore.clearFilters()}>
              Clear filters
            </button>
          </div>
        {:else}
          {#each supabaseStore.filteredSupabase as item (item.id)}
            <SupabaseListItem
              {item}
              isActive={supabaseStore.activeSupabaseId === item.id}
              onclick={() => supabaseStore.setActiveSupabase(item.id)}
              onToggleFavorite={() => supabaseStore.toggleFavorite(item.id)}
            />
          {/each}
        {/if}
      </div>
      ...
      {:else}
        <div class="no-item-state" in:fly={{ y: 20, duration: 400, delay: 150 }}>
          <div class="empty-icon-wrap-large">
            <Icon icon="simple-icons:supabase" width={48} height={48} />
          </div>
          <div class="empty-text">
