<script lang="ts">
  ...
  import PNPMListItem from '$lib/components/pnpm/PNPMListItem.svelte';
  ...
  $effect(() => {
    pnpmStore.setSearchQuery(appStore.searchQuery);
  });
  ...
  <div class="pnpm-body">
    <aside class="list-panel">
      ...
      <div class="list-scroll">
        {#if appStore.pnpm.length === 0}
          <div class="empty-state" in:fly={{ y: 20, duration: 400, delay: 100 }}>
            <div class="empty-icon-wrap">
              <Icon icon="simple-icons:pnpm" width={32} height={32} />
            </div>
        ...
        {:else if pnpmStore.filteredPnpm.length === 0}
          <div class="empty-state" in:fly={{ y: 20, duration: 400 }}>
            ...
            <button class="clear-filters-btn" onclick={() => pnpmStore.clearFilters()}>
              Clear filters
            </button>
          </div>
        {:else}
          {#each pnpmStore.filteredPnpm as item (item.id)}
            <PNPMListItem
              {item}
              isActive={pnpmStore.activePnpmId === item.id}
              onclick={() => pnpmStore.setActivePnpm(item.id)}
              onToggleFavorite={() => pnpmStore.toggleFavorite(item.id)}
            />
          {/each}
        {/if}
      </div>
      ...
      {:else}
        <div class="no-item-state" in:fly={{ y: 20, duration: 400, delay: 150 }}>
          <div class="empty-icon-wrap-large">
            <Icon icon="simple-icons:pnpm" width={48} height={48} />
          </div>
          <div class="empty-text">
