<script lang="ts">
  ...
  import type { TerminalItem } from '$lib/types/index.js';
  ...
  const renderedContent = $derived(renderMarkdown(item.content));
  ...
  $effect(() => {
    ...
    const itemId = terminalStore.activeTerminalId;
    if (!itemId) return;
    if (debounceTimer !== null) {
      clearTimeout(debounceTimer);
      debounceTimer = null;
    }
    const updates: Partial<TerminalItem> = {};
    if (textareaRef) updates.content = textareaRef.value;
    if (titleInputRef) updates.title = titleInputRef.value;
    appStore.updateTerminal(itemId, Object.keys(updates).length > 0 ? updates : {});
    savedFlash = true;
  ...
  function handleTitleInput(e: Event): void {
    ...
    appStore.updateTerminal(itemId, { title: value });
  }
  ...
  function handleContentInput(e: Event): void {
    ...
    debounceTimer = setTimeout(() => {
      appStore.updateTerminal(itemId, { content: value });
    }, 300);
  }
  ...
  function handleTextareaKeydown(e: KeyboardEvent): void {
    if (e.key === 'Tab') {
      ...
      appStore.updateTerminal(itemId, { content: newValue });
  ...
  function handleAddTag(tag: string): void {
    const itemId = terminalStore.activeTerminalId;
    if (!itemId) return;
    const current = appStore.terminal.find((x) => x.id === itemId);
    if (current) appStore.updateTerminal(itemId, { tags: [...(current.tags ?? []), tag] });
  }
  ...
  function handleRemoveTag(tag: string): void {
    ...
    if (!itemId) return;
    const current = appStore.terminal.find((x) => x.id === itemId);
    if (current) appStore.updateTerminal(itemId, { tags: (current.tags ?? []).filter((t) => t !== tag) });
  }

  function handleToggleFavorite(): void {
    const itemId = terminalStore.activeTerminalId;
    if (itemId) terminalStore.toggleFavorite(itemId);
  }

  function handleDuplicate(): void {
    const itemId = terminalStore.activeTerminalId;
    if (!itemId) return;
    const newId = terminalStore.duplicateTerminal(itemId);
    if (newId) terminalStore.setActiveTerminal(newId);
  }
  ...
  function handleDeleteClick(): void {
    ...
    if (!itemId) return;
    if (deleteConfirm) {
      appStore.deleteTerminal(itemId);
      terminalStore.setActiveTerminal(null);
      toastStore.success('Terminal deleted');
      deleteConfirm = false;
    } else {
      deleteConfirm = true;
  ...
  <div class="editor-header">
    <div class="editor-meta">
      <input
        ...
        id="terminal-title-input"
        name="terminal-title"
        class="title-input selectable"
        type="text"
        placeholder="Untitled Terminal"
        value={item.title}
        oninput={handleTitleInput}
      />
      <TagInput tags={item.tags} onAdd={handleAddTag} onRemove={handleRemoveTag} />
    </div>
    ...
    <div class="editor-actions">
      ...
      <div class="view-toggle">
        {#each (['edit', 'split', 'preview'] as ViewMode[]) as mode (mode)}
          <button
            class="view-btn"
      ...
      <button class="action-btn" onclick={handleDuplicate} aria-label="Duplicate Terminal" title="Duplicate">
      ...
      <button
        ...
        aria-label={deleteConfirm ? 'Confirm delete' : 'Delete Terminal'}
        title={deleteConfirm ? 'Click again to confirm' : 'Delete Terminal'}
      >
        {#if deleteConfirm}
