<script lang="ts">
  ...
  import type { SupabaseItem } from '$lib/types/index.js';
  ...
  $effect(() => {
    ...
    const updates: Partial<SupabaseItem> = {};
    if (textareaRef) updates.content = textareaRef.value;
    if (titleInputRef) updates.title = titleInputRef.value;
    appStore.updateSupabase(itemId, Object.keys(updates).length > 0 ? updates : {});
    savedFlash = true;
  ...
  function handleTitleInput(e: Event): void {
    ...
    appStore.updateSupabase(itemId, { title: value });
  }
  ...
  function handleContentInput(e: Event): void {
    ...
    debounceTimer = setTimeout(() => {
      appStore.updateSupabase(itemId, { content: value });
    }, 300);
  }
  ...
  function handleTextareaKeydown(e: KeyboardEvent): void {
    if (e.key === 'Tab') {
      ...
      appStore.updateSupabase(itemId, { content: newValue });
  ...
  function handleAddTag(tag: string): void {
    const itemId = supabaseStore.activeSupabaseId;
    if (!itemId) return;
    const current = appStore.supabase.find((x) => x.id === itemId);
    if (current) appStore.updateSupabase(itemId, { tags: [...(current.tags ?? []), tag] });
  }
  ...
  function handleRemoveTag(tag: string): void {
    ...
    if (!itemId) return;
    const current = appStore.supabase.find((x) => x.id === itemId);
    if (current) appStore.updateSupabase(itemId, { tags: (current.tags ?? []).filter((t) => t !== tag) });
  }
  ...
  <div class="editor-header">
    <div class="editor-meta">
      <input
        ...
        id="supabase-title-input"
        name="supabase-title"
        class="title-input selectable"
        type="text"
        placeholder="Untitled Supabase"
        value={item.title}
