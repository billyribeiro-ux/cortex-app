<script lang="ts">
  ...
  import type { GithubItem } from '$lib/types/index.js';
  ...
  $effect(() => {
    ...
    const itemId = githubStore.activeGithubId;
    if (!itemId) return;
    if (debounceTimer !== null) {
      clearTimeout(debounceTimer);
      debounceTimer = null;
    }
    const updates: Partial<GithubItem> = {};
    if (textareaRef) updates.content = textareaRef.value;
    if (titleInputRef) updates.title = titleInputRef.value;
    appStore.updateGithub(itemId, Object.keys(updates).length > 0 ? updates : {});
    savedFlash = true;
  ...
  function handleTitleInput(e: Event): void {
    ...
    appStore.updateGithub(itemId, { title: value });
  }
  ...
  function handleContentInput(e: Event): void {
    ...
    debounceTimer = setTimeout(() => {
      appStore.updateGithub(itemId, { content: value });
    }, 300);
  }
  ...
  function handleTextareaKeydown(e: KeyboardEvent): void {
    if (e.key === 'Tab') {
      ...
      appStore.updateGithub(itemId, { content: newValue });
  ...
  function handleAddTag(tag: string): void {
    const itemId = githubStore.activeGithubId;
    if (!itemId) return;
    const current = appStore.github.find((x) => x.id === itemId);
    if (current) appStore.updateGithub(itemId, { tags: [...(current.tags ?? []), tag] });
  }
  ...
  function handleRemoveTag(tag: string): void {
    ...
    if (!itemId) return;
    const current = appStore.github.find((x) => x.id === itemId);
    if (current) appStore.updateGithub(itemId, { tags: (current.tags ?? []).filter((t) => t !== tag) });
  }

  function handleToggleFavorite(): void {
    const itemId = githubStore.activeGithubId;
    if (itemId) githubStore.toggleFavorite(itemId);
  }

  function handleDuplicate(): void {
    const itemId = githubStore.activeGithubId;
    if (!itemId) return;
    const newId = githubStore.duplicateGithub(itemId);
    if (newId) githubStore.setActiveGithub(newId);
  }
  ...
  function handleDeleteClick(): void {
    ...
    if (!itemId) return;
    if (deleteConfirm) {
      appStore.deleteGithub(itemId);
      githubStore.setActiveGithub(null);
      toastStore.success('GitHub deleted');
      deleteConfirm = false;
    } else {
      deleteConfirm = true;
  ...
  <div class="editor-header">
    <div class="editor-meta">
      <input
        ...
        id="github-title-input"
        name="github-title"
        class="title-input selectable"
        type="text"
        placeholder="Untitled GitHub"
        value={item.title}
    ...
    <div class="editor-actions">
      ...
      <button class="action-btn" onclick={handleDuplicate} aria-label="Duplicate GitHub" title="Duplicate">
      ...
      <button
        ...
        aria-label={deleteConfirm ? 'Confirm delete' : 'Delete GitHub'}
        title={deleteConfirm ? 'Click again to confirm' : 'Delete GitHub'}
      >
        {#if deleteConfirm}
  ...
  <div class="editor-body" class:split={viewMode === 'split'}>
    {#if viewMode === 'edit' || viewMode === 'split'}
      <textarea
        ...
        id="github-content-input"
        name="github-content"
        class="markdown-textarea selectable"
        class:full={viewMode === 'edit'}
        placeholder="Start writing your GitHub..."
        value={item.content}
