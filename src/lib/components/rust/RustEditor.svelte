<script lang="ts">
  ...
  import type { RustItem } from '$lib/types/index.js';
  ...
  $effect(() => {
    ...
    const itemId = rustStore.activeRustId;
    if (!itemId) return;
    if (debounceTimer !== null) {
      clearTimeout(debounceTimer);
      debounceTimer = null;
    }
    const updates: Partial<RustItem> = {};
    if (textareaRef) updates.content = textareaRef.value;
    if (titleInputRef) updates.title = titleInputRef.value;
    appStore.updateRust(itemId, Object.keys(updates).length > 0 ? updates : {});
    savedFlash = true;
  ...
  function handleTitleInput(e: Event): void {
    ...
    appStore.updateRust(itemId, { title: value });
  }
  ...
  function handleContentInput(e: Event): void {
    ...
    debounceTimer = setTimeout(() => {
      appStore.updateRust(itemId, { content: value });
    }, 300);
  }
  ...
  function handleTextareaKeydown(e: KeyboardEvent): void {
    if (e.key === 'Tab') {
      ...
      appStore.updateRust(itemId, { content: newValue });
  ...
  function handleAddTag(tag: string): void {
    const itemId = rustStore.activeRustId;
    if (!itemId) return;
    const current = appStore.rust.find((x) => x.id === itemId);
    if (current) appStore.updateRust(itemId, { tags: [...(current.tags ?? []), tag] });
  }
  ...
  function handleRemoveTag(tag: string): void {
    ...
    if (!itemId) return;
    const current = appStore.rust.find((x) => x.id === itemId);
    if (current) appStore.updateRust(itemId, { tags: (current.tags ?? []).filter((t) => t !== tag) });
  }

  function handleToggleFavorite(): void {
    const itemId = rustStore.activeRustId;
    if (itemId) rustStore.toggleFavorite(itemId);
  }

  function handleDuplicate(): void {
    const itemId = rustStore.activeRustId;
    if (!itemId) return;
    const newId = rustStore.duplicateRust(itemId);
    if (newId) rustStore.setActiveRust(newId);
  }
  ...
  function handleDeleteClick(): void {
    ...
    if (!itemId) return;
    if (deleteConfirm) {
      appStore.deleteRust(itemId);
      rustStore.setActiveRust(null);
      toastStore.success('Rust deleted');
      deleteConfirm = false;
    } else {
      deleteConfirm = true;
...
<div class="editor-wrap">
  <div class="editor-header">
    ...
    <div class="editor-actions">
      ...
      <button class="action-btn" onclick={handleDuplicate} aria-label="Duplicate Rust" title="Duplicate">
      ...
      <button
        ...
        aria-label={deleteConfirm ? 'Confirm delete' : 'Delete Rust'}
        title={deleteConfirm ? 'Click again to confirm' : 'Delete Rust'}
      >
        {#if deleteConfirm}
