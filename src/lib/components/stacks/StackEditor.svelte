<script lang="ts">
  ...
  const renderedContent = $derived(renderMarkdown(stack.content));
  ...
  $effect(() => {
    ...
    const stackId = stacksStore.activeStackId;
    if (!stackId) return;
    ...
    appStore.updateStack(stackId, Object.keys(updates).length > 0 ? updates : {});
    savedFlash = true;
  ...
  function handleTitleInput(e: Event): void {
    ...
    appStore.updateStack(stackId, { title: value });
  }
  ...
  function handleContentInput(e: Event): void {
    ...
    debounceTimer = setTimeout(() => {
      appStore.updateStack(stackId, { content: value });
    }, 300);
  }
  ...
  function handleTextareaKeydown(e: KeyboardEvent): void {
    if (e.key === 'Tab') {
      ...
      appStore.updateStack(stackId, { content: newValue });
  ...
  function handleAddTag(tag: string): void {
    const stackId = stacksStore.activeStackId;
    if (!stackId) return;
    const current = appStore.stacks.find((s) => s.id === stackId);
    if (current) appStore.updateStack(stackId, { tags: [...(current.tags ?? []), tag] });
  }
  ...
  function handleRemoveTag(tag: string): void {
    ...
    if (!stackId) return;
    const current = appStore.stacks.find((s) => s.id === stackId);
    if (current) appStore.updateStack(stackId, { tags: (current.tags ?? []).filter((t) => t !== tag) });
  }

  function handleToggleFavorite(): void {
    const stackId = stacksStore.activeStackId;
    if (stackId) stacksStore.toggleFavorite(stackId);
  }

  function handleDuplicate(): void {
    const stackId = stacksStore.activeStackId;
    if (!stackId) return;
    const newId = stacksStore.duplicateStack(stackId);
    if (newId) stacksStore.setActiveStack(newId);
  }
  ...
  function handleDeleteClick(): void {
    ...
    if (!stackId) return;
    if (deleteConfirm) {
      appStore.deleteStack(stackId);
      stacksStore.setActiveStack(null);
      toastStore.success('Stack deleted');
      deleteConfirm = false;
    } else {
      deleteConfirm = true;
...
<div class="editor-wrap">
  ...
  <div class="editor-header">
    <div class="editor-meta">
      ...
      />
      <TagInput tags={stack.tags} onAdd={handleAddTag} onRemove={handleRemoveTag} />
    </div>
    ...
    <div class="editor-actions">
      ...
      >
        <Icon icon={stack.isFavorite ? 'ph:star-fill' : 'ph:star'} width={16} height={16} />
      </button>

      <button class="action-btn" onclick={handleDuplicate} aria-label="Duplicate stack" title="Duplicate">
      ...
      <button
        ...
        aria-label={deleteConfirm ? 'Confirm delete' : 'Delete stack'}
        title={deleteConfirm ? 'Click again to confirm' : 'Delete stack'}
      >
        {#if deleteConfirm}
