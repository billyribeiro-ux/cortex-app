<script lang="ts">
  ...
  import type { Prompt } from '$lib/types/index.js';
  ...
  const renderedContent = $derived(renderMarkdown(prompt.content));
  ...
  $effect(() => {
    ...
    appStore.updatePrompt(promptId, Object.keys(updates).length > 0 ? updates : {});
    savedFlash = true;
  ...
  function handleTitleInput(e: Event): void {
    ...
    appStore.updatePrompt(promptId, { title: value });
  }
  ...
  function handleContentInput(e: Event): void {
    ...
    debounceTimer = setTimeout(() => {
      appStore.updatePrompt(promptId, { content: value });
    }, 300);
  }
  ...
  function handleTextareaKeydown(e: KeyboardEvent): void {
    if (e.key === 'Tab') {
      ...
      appStore.updatePrompt(promptId, { content: newValue });
  ...
  function handleAddTag(tag: string): void {
    const promptId = promptsStore.activePromptId;
    if (!promptId) return;
    const current = appStore.prompts.find((p) => p.id === promptId);
    if (current) appStore.updatePrompt(promptId, { tags: [...(current.tags ?? []), tag] });
  }
  ...
  function handleRemoveTag(tag: string): void {
    ...
    if (!promptId) return;
    const current = appStore.prompts.find((p) => p.id === promptId);
    if (current) appStore.updatePrompt(promptId, { tags: (current.tags ?? []).filter((t) => t !== tag) });
  }
...
<div class="editor-wrap">
  ...
  <div class="editor-header">
    <div class="editor-meta">
      ...
      />
      <TagInput tags={prompt.tags} onAdd={handleAddTag} onRemove={handleRemoveTag} />
    </div>
