<script lang="ts">
  ...
  import type { DevItem } from '$lib/types/index.js';
  ...
  $effect(() => {
    ...
    const itemId = devStore.activeDevId;
    if (!itemId) return;
    if (debounceTimer !== null) {
      clearTimeout(debounceTimer);
      debounceTimer = null;
    }
    const updates: Partial<DevItem> = {};
    if (textareaRef) updates.content = textareaRef.value;
    if (titleInputRef) updates.title = titleInputRef.value;
    appStore.updateDev(itemId, Object.keys(updates).length > 0 ? updates : {});
    savedFlash = true;
  ...
  function handleTitleInput(e: Event): void {
    ...
    appStore.updateDev(itemId, { title: value });
  }
  ...
  function handleContentInput(e: Event): void {
    ...
    debounceTimer = setTimeout(() => {
      appStore.updateDev(itemId, { content: value });
    }, 300);
  }
  ...
  function handleTextareaKeydown(e: KeyboardEvent): void {
    if (e.key === 'Tab') {
      ...
      appStore.updateDev(itemId, { content: newValue });
  ...
  function handleAddTag(tag: string): void {
    const itemId = devStore.activeDevId;
    if (!itemId) return;
    const current = appStore.dev.find((x) => x.id === itemId);
    if (current) appStore.updateDev(itemId, { tags: [...(current.tags ?? []), tag] });
  }
  ...
  function handleRemoveTag(tag: string): void {
    ...
    if (!itemId) return;
    const current = appStore.dev.find((x) => x.id === itemId);
    if (current) appStore.updateDev(itemId, { tags: (current.tags ?? []).filter((t) => t !== tag) });
  }

  function handleToggleFavorite(): void {
    const itemId = devStore.activeDevId;
    if (itemId) devStore.toggleFavorite(itemId);
  }

  function handleDuplicate(): void {
    const itemId = devStore.activeDevId;
    if (!itemId) return;
    const newId = devStore.duplicateDev(itemId);
    if (newId) devStore.setActiveDev(newId);
  }
  ...
  function handleDeleteClick(): void {
    ...
    if (!itemId) return;
    if (deleteConfirm) {
      appStore.deleteDev(itemId);
      devStore.setActiveDev(null);
      toastStore.success('Dev deleted');
      deleteConfirm = false;
    } else {
      deleteConfirm = true;
...
<div class="editor-wrap">
  <div class="editor-header">
    ...
    <div class="editor-actions">
      ...
      <button class="action-btn" onclick={handleDuplicate} aria-label="Duplicate Dev" title="Duplicate">
      ...
      <button
        ...
        aria-label={deleteConfirm ? 'Confirm delete' : 'Delete Dev'}
        title={deleteConfirm ? 'Click again to confirm' : 'Delete Dev'}
      >
        {#if deleteConfirm}
