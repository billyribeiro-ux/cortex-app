<script lang="ts">
  ...
  import type { VercelItem } from '$lib/types/index.js';
  ...
  function handleTitleInput(e: Event): void {
    ...
    appStore.updateVercel(itemId, { title: value });
  }
  ...
  function handleContentInput(e: Event): void {
    ...
    debounceTimer = setTimeout(() => {
      appStore.updateVercel(itemId, { content: value });
    }, 300);
  }
  ...
  function handleTextareaKeydown(e: KeyboardEvent): void {
    if (e.key === 'Tab') {
      ...
      appStore.updateVercel(itemId, { content: newValue });
  ...
  function handleAddTag(tag: string): void {
    const itemId = vercelStore.activeVercelId;
    if (!itemId) return;
    const current = appStore.vercel.find((x) => x.id === itemId);
    if (current) appStore.updateVercel(itemId, { tags: [...(current.tags ?? []), tag] });
  }
  ...
  function handleRemoveTag(tag: string): void {
    ...
    if (!itemId) return;
    const current = appStore.vercel.find((x) => x.id === itemId);
    if (current) appStore.updateVercel(itemId, { tags: (current.tags ?? []).filter((t) => t !== tag) });
  }
  ...
  <div class="editor-header">
    <div class="editor-meta">
      <input
        ...
        id="vercel-title-input"
        name="vercel-title"
        class="title-input selectable"
        type="text"
        placeholder="Untitled Vercel"
        value={item.title}
  ...
  <div class="editor-body" class:split={viewMode === 'split'}>
    {#if viewMode === 'edit' || viewMode === 'split'}
      <textarea
        ...
        id="vercel-content-input"
        name="vercel-content"
        class="markdown-textarea selectable"
        class:full={viewMode === 'edit'}
        placeholder="Start writing your Vercel..."
        value={item.content}
