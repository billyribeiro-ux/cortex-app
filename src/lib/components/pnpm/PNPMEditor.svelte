<script lang="ts">
  ...
  $effect(() => {
    ...
    const itemId = pnpmStore.activePnpmId;
    if (!itemId) return;
    if (debounceTimer !== null) {
      clearTimeout(debounceTimer);
      debounceTimer = null;
    }
    const updates: Partial<PnpmItem> = {};
    if (textareaRef) updates.content = textareaRef.value;
    if (titleInputRef) updates.title = titleInputRef.value;
    appStore.updatePnpm(itemId, Object.keys(updates).length > 0 ? updates : {});
    savedFlash = true;
  ...
  function handleTitleInput(e: Event): void {
    ...
    appStore.updatePnpm(itemId, { title: value });
  }
  ...
  function handleContentInput(e: Event): void {
    ...
    debounceTimer = setTimeout(() => {
      appStore.updatePnpm(itemId, { content: value });
    }, 300);
  }
  ...
  function handleTextareaKeydown(e: KeyboardEvent): void {
    if (e.key === 'Tab') {
      ...
      appStore.updatePnpm(itemId, { content: newValue });
  ...
  function handleAddTag(tag: string): void {
    const itemId = pnpmStore.activePnpmId;
    if (!itemId) return;
    const current = appStore.pnpm.find((x) => x.id === itemId);
    if (current) appStore.updatePnpm(itemId, { tags: [...(current.tags ?? []), tag] });
  }
  ...
  function handleRemoveTag(tag: string): void {
    ...
    if (!itemId) return;
    const current = appStore.pnpm.find((x) => x.id === itemId);
    if (current) appStore.updatePnpm(itemId, { tags: (current.tags ?? []).filter((t) => t !== tag) });
  }

  function handleToggleFavorite(): void {
    const itemId = pnpmStore.activePnpmId;
    if (itemId) pnpmStore.toggleFavorite(itemId);
  }

  function handleDuplicate(): void {
    const itemId = pnpmStore.activePnpmId;
    if (!itemId) return;
    const newId = pnpmStore.duplicatePnpm(itemId);
    if (newId) pnpmStore.setActivePnpm(newId);
  }
  ...
  function handleDeleteClick(): void {
    ...
    if (!itemId) return;
    if (deleteConfirm) {
      appStore.deletePnpm(itemId);
      pnpmStore.setActivePnpm(null);
      toastStore.success('PNPM deleted');
      deleteConfirm = false;
    } else {
      deleteConfirm = true;
...
<div class="editor-wrap">
  <div class="editor-header">
    ...
    <div class="editor-actions">
      ...
      <button class="action-btn" onclick={handleDuplicate} aria-label="Duplicate PNPM" title="Duplicate">
      ...
      <button
        ...
        aria-label={deleteConfirm ? 'Confirm delete' : 'Delete PNPM'}
        title={deleteConfirm ? 'Click again to confirm' : 'Delete PNPM'}
      >
        {#if deleteConfirm}
